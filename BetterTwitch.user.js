...
(function () {
  ...
  const SETTING_HELP = {
    en: {
      ...
      pingSound: 'Chooses which of the 18 built-in short sounds BetterTwitch uses for mention and direct-reply notifications. Use the Test button to hear the current choice immediately before relying on it in chat. The sound uses a stable pitch and is generated locally; it does not change any Twitch audio.',
      pingVolume: 'Controls only the volume of BetterTwitch mention and reply sounds, from 0% (completely silent) to 100% (full notification volume). Use the Test button to check a comfortable level. This setting does not change the stream, Twitch ads, browser volume, or operating-system volume.',
    ...
    de: {
      ...
      pingSound: 'Wählt einen der 18 eingebauten kurzen Töne für Erwähnungen und direkte Antworten. Mit dem Test-Button kannst du die aktuelle Auswahl sofort anhören, bevor du dich im Chat darauf verlässt. Der Ton hat eine stabile Tonhöhe, wird lokal erzeugt und verändert keinerlei Twitch-Audio.',
      pingVolume: 'Regelt ausschließlich die Lautstärke der BetterTwitch-Töne für Erwähnungen und Antworten, von 0 % (vollständig stumm) bis 100 % (volle Benachrichtigungslautstärke). Prüfe mit dem Test-Button eine angenehme Lautstärke. Stream, Twitch-Werbung, Browser-Lautstärke und Systemlautstärke werden nicht verändert.',
    ...
    ru: {
      ...
      pingSound: 'Выбирает один из 18 встроенных коротких звуков для уведомлений об упоминаниях и прямых ответах. Нажмите кнопку «Тест», чтобы сразу услышать текущий вариант и проверить его до использования в чате. Звук создаётся локально с постоянной высотой тона и не меняет аудио Twitch.',
      pingVolume: 'Регулирует только громкость звуков BetterTwitch для упоминаний и ответов: от 0 % (полная тишина) до 100 % (максимальная громкость уведомления). Используйте кнопку «Тест», чтобы выбрать комфортный уровень. Громкость трансляции, рекламы Twitch, браузера и операционной системы не меняется.',
